  ...
  return (
    <div className="pt-20 pb-5 flex gap-5 justify-around  bg-mine-shaft-950 font-['poppins']">
      ...
      {footerLinks.map((item, index) => (
        <div key={index}>
          <div className="text-lg font-semibold md-4 text-bright-sun-400">{item.title}</div>
          {item.links.map((link, linkIndex) => (
            <div key={linkIndex} className="text-mine-shaft-300 text-sm hover:text-bright-sun-400 cursor-pointer md-1 hover:translate-x-2 transition duration-300 ease-in-out">
              {link}
            </div>
          ))}
        </div>
